...
import { useStaticMailtemplateSearch } from "@/hooks/queries/use-static-mail-template"
...
interface StaticTemplateComboboxProps {
...
export function StaticTemplateCombobox({
  value,
  onChange,
  onBlur,
  error,
  departmentId,
}: StaticTemplateComboboxProps) {
  ...
  const { data, isFetching } = useStaticMailtemplateSearch(
    debouncedSearch,
    shouldFetch,
    deps
  )
  ...
  const resolvedLabel =
    extractLabel(value) ||
    options.find((d: any) => d.id === resolvedValue)?.name
